...
def get_matches(value, txt):
    matches = process.extract(value, txt)
...
def speciesToColumn(group, sp):
    return f"{group}_{sp.lower().replace(', ','_')}"


species_column_names = [speciesToColumn(
    k, sp) for k, v in species.items() for sp in v]
...
def process_line(result, line):
    ...
    system = systems.setdefault(
        system_name, {'x': -1, 'y': -1, 'z': -1, 'num_sites': 0})
    ...
    if len(line) > 2:
        ...
        try:
            ...
            pass
    # Pad with None values until we have the full 14 columns
    line = line + (14-len(line)) * [None]

    if not any([i is None for i in line[11:14]]):
    ...
    if (system['x'], system['y'], system['z']) == (-1, -1, -1):
        if system_name == prev_system_name:
            ...
            (system['x'], system['y'], system['z']
             ) = lookup_coords(system_name)

    line[11] = system['x']
    line[12] = system['y']
    line[13] = system['z']

    # Replace empty strings with None and add the original row number from the
    # google sheet
    line = [rownum] + list(
        map(lambda x: None if isinstance(x, str)
            and x.strip() == '' else x, line))
...
def lookup_coords(system_name):
    url = "https://www.edsm.net/api-v1/system?"\
          f"showCoordinates=1&systemName={system_name}"
...
def read_gsheet():
    ...
    sites = []
    systems = {}
    reduce(process_line, result.get('values', []),
           (systems, sites, start_row-1))
    columns = ['gsheet_rownum', 'added_by', 'system_name', 'mollusc',
               'lagrange_cloud', 'ice_crystal', 'metallic_crystal',
               'silicate_crystal', 'solid_mineral', 'empty1',
               'distance_to_nearest_star', 'empty2', 'x', 'y', 'x']
...
def calculate_columns(systems, sites):
    ...
    for col in species:
        for sp in species[col]:
            sites[f"{col}_{sp.lower().replace(', ','_')}"] = sites.apply(
                lambda row: row[col] is not None and
                sp in get_matches(row[col], species[col]), axis=1)
    agg_columns = dict([(v, np.max) for v in species_column_names])
    agg_columns['added_by'] = lambda x: x.iloc[0]
    systems = systems.join(sites[['system_name', 'added_by'] +
                                 species_column_names]
                           .groupby(by=['system_name']).agg(agg_columns))

    for group in species:
        columns = [k for k in species_column_names if k.startswith(group)]
        systems[group] = systems.apply(lambda row: ", ".join(
            [columnToSpecies(col)[1] for col in columns if row[col]]) or None,
            axis=1)
...
def save_csv(systems):
    systems.to_csv('banana_nebula_nsp.csv')


def save_layers(layers, tables):
    """Save layer PCB and json files"""
...
def generate_layers(systems):
    ...
    json_columns = ['system_name', 'added_by', 'num_sites', 'mollusc',
                    'lagrange_cloud', 'ice_crystal', 'metallic_crystal',
                    'silicate_crystal', 'solid_mineral', 'x', 'y', 'z']

    systems['system_name'] = systems.index
    layer_systems = systems.replace(
        {np.nan: None}).round({'x': 2, 'y': 2, 'z': 2})

    pcd.points = o3d.utility.Vector3dVector(
        layer_systems[['x', 'y', 'z']].to_numpy())
    pcd.colors = o3d.utility.Vector3dVector(
        [color for i in range(len(layer_systems))])
    ...
    for i, group in enumerate(species.keys()):
        ...
        pcd = o3d.geometry.PointCloud()
        layer_systems = systems[systems[group].notnull()].replace(
            {np.nan: None}).round({'x': 2, 'y': 2, 'z': 2})
        pcd.points = o3d.utility.Vector3dVector(
            layer_systems[['x', 'y', 'z']].to_numpy())
        pcd.colors = o3d.utility.Vector3dVector(
            [color[:3] for i in range(len(layer_systems))])
        layers[group] = pcd
...
def main():
    """Main function that imports all NSP data."""
