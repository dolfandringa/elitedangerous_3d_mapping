...
def process_line(result, line):
    ...
    line = [rownum] + list(map(lambda x: None if isinstance(x,
                                                            str) and x.strip() == '' else x, line))
...
def read_gsheet():
    ...
    columns = ['gsheet_rownum', 'added_by', 'system_name', 'mollusc', 'lagrange_cloud', 'ice_crystal', 'metallic_crystal',
               'silicate_crystal', 'solid_mineral', 'empty1', 'distance_to_nearest_star', 'empty2', 'x', 'y', 'x']
...
def calculate_columns(systems, sites):
    ...
    for col in species.keys():
        for sp in species[col]:
            sites[f"{col}_{sp.lower().replace(', ','_')}"] = sites.apply(
                lambda row: row[col] is not None and sp in get_matches(row[col], species[col]), axis=1)
    ...
    systems = systems.join(sites[['system_name', 'added_by'] +
                                 species_column_names].groupby(by=['system_name']).agg(agg_columns))
    for group in species.keys():
        ...
        systems[group] = systems.apply(lambda row: ", ".join(
            [columnToSpecies(col)[1] for col in columns if row[col]]) or None, axis=1)
    return systems, sites
...
def generate_layers(systems):
    ...
    json_columns = ['system_name', 'added_by', 'num_sites', 'mollusc', 'lagrange_cloud',
                    'ice_crystal', 'metallic_crystal', 'silicate_crystal', 'solid_mineral', 'x', 'y', 'z']
